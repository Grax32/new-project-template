...
class ProgramService implements IService {
    ...
    async start(): Promise<void> {
        ...
        const logChanged = (line: string) => {
            ...
            if (healthStatusMatch) {
                console.log(
                    `Program ${this.name} health status changed to: ${healthStatusMatch.status}`,
                );
    ...
    async stop(): Promise<void> {
        ...
        const timeout = setTimeout(async () => {
            const shouldKill =
                child.exitCode == null &&
                (child.signalCode == null || child.signalCode !== 'SIGKILL');
            console.log(
...
class ProgramServiceGroup implements IServiceGroup {
    ...
    private loadServices(): ProgramService[] {
        ...
        const result = Object.entries(programsConfig).map(
            ([id, cfg]) => new ProgramService(cfg, id),
        );
        return result;
    }
