...
process.stdin.on('data', (chunk) => {
    ...
    const containsSuccessMessage =
        chunkString.includes(expectedSuccessMessage) &&
...
process.stdin.on('end', () => {
    ...
    if (exitCode === 1) {
        console.error(
            `Preparation did not complete successfully. Did not receive expected success message of "${expectedSuccessMessage}".`,
        );
    }
