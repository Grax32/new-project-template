...
(async function () {
    ...
    if (startAllProgramsBtn) {
        startAllProgramsBtn.addEventListener('click', () =>
            serviceGroupAction('programs', 'start'),
        );
    }
    ...
    function renderServiceGroup(serviceGroupName, services) {
        ...
        for (const p of services) {
            ...
            stopBtn.addEventListener('click', () =>
                serviceAction(p.serviceGroup, p.serviceId, 'stop'),
            );
            startBtn.addEventListener('click', () =>
                serviceAction(p.serviceGroup, p.serviceId, 'start'),
            );
            restartBtn.addEventListener('click', () =>
                serviceAction(p.serviceGroup, p.serviceId, 'restart'),
            );
    ...
    function refresh() {
        renderServiceGroup(
            'programs',
            state.filter((s) => s.serviceGroup === 'programs'),
        );
        renderServiceGroup(
            'docker',
            state.filter((s) => s.serviceGroup === 'docker'),
        );
    }
    ...
    function updateStartStopButtons() {
        const dockerStatuses = state
            .filter((s) => s.serviceGroup === 'docker')
            .map((s) => s.status);
        ...
        const programsStatuses = state
            .filter((s) => s.serviceGroup === 'programs')
            .map((s) => s.status);
