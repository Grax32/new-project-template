...
async function main() {
  ...
  if (info.code !== 0) {
    ...
    const ans = (
      await prompt('Would you like to try to start Docker Desktop (open Docker UI) now? (y/N): ')
    )
      .trim()
      .toLowerCase();
    if (ans === 'y' || ans === 'yes') {
  ...
  if (!services.ok) {
    ...
    const ans = (
      await prompt('Would you like to bring up the Compose stack (docker compose up -d)? (y/N): ')
    )
      .trim()
      .toLowerCase();
    if (ans === 'y' || ans === 'yes') {
