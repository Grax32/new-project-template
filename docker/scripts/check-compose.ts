...
function listServices() {
    ...
    const allFromConfig = (cfg.code === 0 && cfg.stdout.trim())
    ...
    if ( jsonOut.code === 0 && jsonOut.stdout.trim()) {
        const lines = jsonOut.stdout.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    ...
    if (allFromConfig) {
        for (const s of allFromConfig) if (!(s in parsedStatusMap)) parsedStatusMap[s] = 'not running';
        const runningList = allFromConfig.filter(svc => isRunningState(parsedStatusMap[svc] || ''));
    ...
    const allList = all.stdout.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
...
async function main() {
    ...
    if (info.code !== 0) {
        ...
        const ans = (await prompt('Would you like to try to start Docker Desktop (open Docker UI) now? (y/N): ')).trim().toLowerCase();
        if (ans === 'y' || ans === 'yes') {
    ...
    if (!services.ok) {
        console.log('Could not read docker compose status. This usually means either there is no docker compose.yml in the current directory or Docker Compose returned an error.');
        console.log('Error output:\n', services.error);
        const ans = (await prompt('Would you like to bring up the Compose stack (docker compose up -d)? (y/N): ')).trim().toLowerCase();
        if (ans === 'y' || ans === 'yes') {
    ...
    if (all.length === 0) {
        ...
        const ans = (await prompt('Would you like to run `docker compose up -d` now? (y/N): ')).trim().toLowerCase();
        if (ans === 'y' || ans === 'yes') {
            const up = run('docker', ['compose', 'up', '-d']);
            if (up.code !== 0) {
                console.error('Failed to start compose stack:', up.stderr || up.stdout);
    ...
    if (runningCount < all.length) {
        ...
        const ans = (await prompt('Restart the compose stack (docker compose restart) ? (y/N): ')).trim().toLowerCase();
        if (ans === 'y' || ans === 'yes') {
