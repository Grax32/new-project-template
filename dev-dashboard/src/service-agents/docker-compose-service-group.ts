...
const statusMappingLookupMap: Record<string, ServiceStatus> = Object.fromEntries([
  ...
  ...statusMappingDefinition
    .flatMap((s) => s.alternates)
    .map((a) => [a, statusMappingDefinition.find((s) => s.alternates.includes(a))?.primary]),
]);
...
class DockerComposeService implements IService {
  ...
  constructor(public readonly serviceId: string) {
    if (!serviceId) {
      throw new Error(
        'DockerComposeService requires a serviceId.  This will be a docker compose service name.',
      );
    }
  ...
  async state(): Promise<ServiceState> {
    ...
    const status = this.statusOverride
      ? this.statusOverride
      : this.normalizeStatus(container ? container.State : null);
  ...
  async logs(tail = 100): Promise<string> {
    try {
      const result = await executeDockerCommand(
        `compose logs --since=${config.startTime.toISOString()} --tail=${tail} ${this.serviceId}`,
      );
      ...
      const output =
        `Log entries prior to start time of ${startTimeStr} can be viewed in the Docker UI or
...
class DockerComposeServiceGroup implements IServiceGroup {
  private readonly dockerServices: Promise<DockerComposeService[]> = new Lazy(() =>
  ...
  private async discoverServices(): Promise<DockerComposeService[]> {
    try {
      ...
      const services = configServices.stdout
        .trim()
        .split('\n')
        .filter(Boolean)
...
export const dockerServiceCollection: IServiceGroup = new DockerComposeServiceGroup(
  'docker-compose',
);
