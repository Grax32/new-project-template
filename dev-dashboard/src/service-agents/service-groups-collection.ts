...
import {
  dockerServiceCollection,
  dockerServicesserviceGroup,
} from './docker-compose-service-group';
...
export async function getAllServices(): Promise<
  Array<{ serviceGroup: string; service: IService }>
> {
...
export async function getConfiguredServices(): Promise<ConfiguredService[]> {
  ...
  return [...configuredDockerServices, ...configuredProgramServices];
}
